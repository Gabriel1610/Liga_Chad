...
public class App 
{
    ...
    public static Equipo crearEquipo(Scanner teclado, Liga laLiga){
        ...
        do{
            ...
            try {
                ...
                equipoServicio.crearEquipo(nombreEquipo, laLiga);
            } catch (NumberFormatException e) {
                System.out.println("Entrada inválida. Intente de nuevo.");
                repetir = true; // Indica que debe repetirse el ciclo
            } catch (Exception e) {
                System.out.println("Otra excepción manejada.");
                repetir = true;
            }
        }while(laLiga.buscarEquipoPorNombre(nombreEquipo) != null);
    ...
    public static Equipo solicitarNombreDeUnEquipoExistente(Scanner teclado, Liga laLiga){
        ...
        do{
            ...
            while(nombreEquipo.equals("")){
                System.out.print("No ha ingresado un nombre correcto. Inténtelo nuevamente: ");
                nombreEquipo = teclado.nextLine();
            }
            equipo = laLiga.buscarEquipoPorNombre(nombreEquipo);
            if(equipo == null){
                System.out.print("El equipo ingresado no existe. Ingrese el nombre de un equipo existente: ");
            }    
        }while(equipo == null);
        return equipo;
    }
    ...
    public static void registrarJugador(Scanner teclado, Liga laLiga){
        String esTitular, nombre;
        int edad;
        int cantGoles;
        int minutosJugados;
        int cantPartidosIngresados;
        Equipo equipo;
        equipo = solicitarNombreDeUnEquipoExistente(teclado, laLiga);
        System.out.print("\n¿Se trata de un jugador titular? (S / N): ");
        esTitular = teclado.next();
        boolean jugadorExistente;
        ...
        do{
            ...
            jugadorExistente = laLiga.existeJugador(nombre);
            if(jugadorExistente){
                System.out.print("Ese jugador ya existe. Intente con otro nombre: ");
            }
        }while(jugadorExistente);
        System.out.println("Nombre ingresado: " + nombre);
        System.out.print("\nIngrese la edad del jugador: ");
        edad = teclado.nextInt();
        while(edad < EDAD_MÍNIMA){
            System.out.print("La edad ingresada no es válida. Inténtelo nuevamente: ");
            edad = teclado.nextInt();
        }
        System.out.print("\nIngrese cantidad de goles del jugador: ");
        cantGoles = teclado.nextInt();
        while(cantGoles < MÍN_CANT_GOLES){
            System.out.print("La cantidad de goles ingresada no es válida. Inténtelo nuevamente: ");
            cantGoles = teclado.nextInt();
        }
        if(esTitular.equalsIgnoreCase("S")){
            System.out.print("\nIngrese cantidad de minutos ya jugados por el jugador: ");
            minutosJugados = teclado.nextInt();
            while(minutosJugados < MÍN_CANT_MINUTOS_JUGADOS){
                System.out.print("La cantidad de minutos ingresada no es válida. Inténtelo nuevamente: ");
                minutosJugados = teclado.nextInt();
            }
            ...
            while(cantPartidosIngresados < MÍN_CANT_PARTIDOS_JUGADOS){
                System.out.print("La cantidad de partidos ingresada no es válida. Inténtelo nuevamente: ");
                cantPartidosIngresados = teclado.nextInt();
            }
